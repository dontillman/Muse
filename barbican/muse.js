...
class Muse {
	...
	var muse = this;
	window.addEventListener('resize', function() {muse.layout();});
    }
    ...
    init() {
	if (window.opener) {
	    console.log('parent state: ', window.opener.muse.state);
	    this.state.copy(window.opener.muse.state);
	    this.state.start = 1;
	    console.log('did copy: ', this.state);
	    this.slaveMode = true;
	}
...
class MusePanel {
	...
	// do the current bar
	var bar = this.muse.state.currentBar;
...
function muse(museDivId, presetsDivId) {
    muse = new Muse(museDivId, presetsDivId);
}
